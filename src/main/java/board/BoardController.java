	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "write":	
			if (request.getMethod().equals("GET")) {
				rd = request.getRequestDispatcher("/WEB-INF/view/board/write.jsp");		// CK Editor version
				rd.forward(request, response);
				//response.sendRedirect("/ncpl/WEB-INF/view/board/write.jsp");
			} else {
				...
				response.sendRedirect("/ncpl/board/list?p=1&f=&q=");
			}
			break;
		...
		case "reply":
			...
			response.sendRedirect("/ncpl/board/detail?bid=" + bid + "&uid=" + uid + "&option=DNI");
			break;
		...
		case "deleteConfirm":
			bid = Integer.parseInt(request.getParameter("bid"));
			dao.deleteBoard(bid);
			response.sendRedirect("/ncpl/board/list?p=" + session.getAttribute("currentBoardPage") + "&f=&q=");
			break;
		...
		case "update":	//업데이트와 같이 GET & POST 방식이 정석이긴 함
			if (request.getMethod().equals("GET")) {
				...
				request.setAttribute("board", board);
				rd = request.getRequestDispatcher("/WEB-INF/view/board/update.jsp"); // CK Editor version
				...
				response.sendRedirect("/ncpl/board/detail?bid=" + bid + "&uid=" + uid + "&option=DNI"); 
				//response.sendRedirect("/ncpl/board/detail?bid=" + bid + "&uid=" + uid); 도 가능
