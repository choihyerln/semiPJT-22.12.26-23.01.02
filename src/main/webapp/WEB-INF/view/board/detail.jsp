...
<body>
	...
	<div class="container">
		<div class="row mt-5 mb-5">			
			...
			<h4 class="page-small-subtitle"><strong>게시글 상세 조회</strong>
		    <span style="font-size: 0.6em;">
				<a href="/ncpl/board/list?p=${currentBoardPage}&f=&q=" class="ms-5"><i class="fas fa-list-ul"></i>목록</a>
			<!-- 본인만 수정 가능 -->
				<c:if test="${board.uid eq uid}">
					<a href="/ncpl/board/update?bid=${board.bid}" class="ms-3"><i class="far fa-edit"></i>수정</a>
				</c:if>
				...
				<!-- 본인만 삭제 가능 -->
				<c:if test="${board.uid eq uid}">
					<a href="/ncpl/board/delete?bid=${board.bid}" class="ms-3"><i class="fas fa-trash-alt"></i>삭제</a>
				</c:if>
				<c:if test="${board.uid ne uid}">
			...
			<div class="row">
			    <div class="col-8">
		        <h5 id="midP">${board.title}</h5>
				<h6>글 번호: ${board.bid} | ${fn:replace(board.modTime, 'T', ' ')}</h6>
				<h6>첨부 파일: 
				<c:forEach var="file" items="${fileList}">
					<a href="/ncpl/board/download?file=${file}" class="me-2" download>${file}</a>
				</c:forEach>
			    </h6>
			</div>
			<div class="col-4 text-end">
			    <h5 id="midP">${board.uname}</h5>
			...
			<!-- 댓글 -->
				...
				<form class="form-inline" action="/ncpl/board/reply" method="post">
					<input type="hidden" name="bid" value="${board.bid}">     <!-- bid -->
					<input type="hidden" name="uid" value="${board.uid}">     <!-- uid -->
					<table class="table table-borderless mt-2">
					<tr class="d-flex">
						<td class="col-1 text-end">
							<label for="content" id="midP">댓글</label>
						</td>
						<td class="col-9">
							<textarea class="form-control" id="content" name="content" rows="3"></textarea>
						</td>
						<td class="col-2">
							<button type="submit" class="btn btn-orange btn-mini">   제출   </button>
						</td>
					</tr>
					</table>
				</form>
